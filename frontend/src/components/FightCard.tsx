    ...
    return [
        {
            axis: 'Striking',
            f1: normalize(
                f1.sig_strikes_landed_per_minute,
                'sig_strikes_landed_per_minute',
            ),
            f2: normalize(
                f2.sig_strikes_landed_per_minute,
                'sig_strikes_landed_per_minute',
            ),
        },
        {
    ...
    return (
        <div className="grid grid-cols-[1fr_6rem_1fr] items-center gap-4 py-2.5">
            <div className="flex flex-col items-end gap-1.5">
                ...
                >
                    <div
                        ...
                        style={{
                            width: `${f1Pct}%`,
                            background: `linear-gradient(to left, ${F1_COLOR}, #FF8F00)`,
                        }}
                    />
                </div>
            </div>
            ...
            <div className="flex flex-col items-center gap-0.5">
                ...
                <div
                    className="relative w-full h-0.5 rounded-full mt-0.5"
            ...
            <div className="flex flex-col items-start gap-1.5">
                ...
                >
                    <div
                        ...
                        style={{
                            width: `${f2Pct}%`,
                            background: `linear-gradient(to right, ${F2_COLOR}, #1E88E5)`,
                        }}
                    />
                </div>
            </div>
...
function FighterCol({ fighter, color, align }: FighterColProps) {
    ...
    return (
        ...
        >
            ...
            <div
                className={`flex gap-3 text-sm font-bold ${end ? 'flex-row-reverse' : ''}`}
            >
                <span style={{ color: '#66BB6A' }}>{fighter.wins}W</span>
                <span style={{ color: '#EF5350' }}>{fighter.losses}L</span>
                {fighter.draws > 0 && (
                    <span style={{ color: '#9E9E9E' }}>{fighter.draws}D</span>
                )}
            </div>

            <div
                className={`flex flex-col gap-1 text-xs ${end ? 'items-end' : ''}`}
                style={{ color: '#9E9E9E' }}
            >
    ...
    return (
        <div className="w-[80%] mx-auto py-8 flex flex-col gap-5">
            ...
            <div className="rounded-2xl p-6" style={glass}>
                ...
                <div
                    className="divide-y"
                    style={{ borderColor: 'rgba(255,179,0,0.06)' }}
                >
                    <StatRow
                        label="Sig. Str / Min"
                        f1Value={fighter1.sig_strikes_landed_per_minute}
                        f2Value={fighter2.sig_strikes_landed_per_minute}
                        statKey="sig_strikes_landed_per_minute"
                        fmt={(v) => v.toFixed(2)}
                    />
                    <StatRow
                        label="Str. Accuracy"
                        f1Value={fighter1.sig_strike_accuracy}
                        f2Value={fighter2.sig_strike_accuracy}
                        statKey="sig_strike_accuracy"
                        fmt={(v) => `${v}%`}
                    />
                    <StatRow
                        label="Takedown Avg"
                        f1Value={fighter1.takedown_average}
                        f2Value={fighter2.takedown_average}
                        statKey="takedown_average"
                        fmt={(v) => v.toFixed(2)}
                    />
                    <StatRow
                        label="Submission Avg"
                        f1Value={fighter1.submission_average}
                        f2Value={fighter2.submission_average}
                        statKey="submission_average"
                        fmt={(v) => v.toFixed(2)}
                    />
                    <StatRow
                        label="KO %"
                        f1Value={fighter1.knockout_percentage}
                        f2Value={fighter2.knockout_percentage}
                        statKey="knockout_percentage"
                        fmt={(v) => `${v}%`}
                    />
                    <StatRow
                        label="TKO %"
                        f1Value={fighter1.technical_knockout_percentage}
                        f2Value={fighter2.technical_knockout_percentage}
                        statKey="technical_knockout_percentage"
                        fmt={(v) => `${v}%`}
                    />
                    <StatRow
                        label="Decision %"
                        f1Value={fighter1.decision_percentage}
                        f2Value={fighter2.decision_percentage}
                        statKey="decision_percentage"
                        fmt={(v) => `${v}%`}
                    />
                </div>
            </div>
